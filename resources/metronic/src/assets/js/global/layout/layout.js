...
var KTLayout = function() {
	...
	var initHeaderMenu = function() {
		...
		headerMenuOffcanvas = new KTOffcanvas('kt_header_menu_wrapper', {
			...
			toggleBy: {
				...
				state: 'kt-header-mobile_toolbar-toggle-active'
			}
		});
	...
	var initHeaderTopbar = function() {
		asideToggler = new KTToggle('kt_header_mobile_topbar_toggle', {
			target: 'body',
			targetState: 'topbar-mobile-on',
			togglerState: 'active'
			// togglerState: 'kt-header-mobile_toolbar-topbar-toggle-active'
		});
	}
	...
	var initAside = function() {
		...
		var asideOffcanvasClass = KTUtil.hasClass(aside, 'aside-offcanvas-default') ? 'aside-offcanvas-default' : 'aside';
		...
		asideMenuOffcanvas = new KTOffcanvas('kt_aside', {
			...
			toggleBy: {
				...
				state: 'kt-header-mobile-toolbar-toggle-active'
			}
		});

		// Handle minimzied aside hover
		if (KTUtil.hasClass(body, 'aside-fixed')) {
			var insideTm;
			var outsideTm;
			...
			KTUtil.addEvent(aside, 'mouseenter', function(e) {
				...
				insideTm = setTimeout(function() {
					if (KTUtil.hasClass(body, 'aside-minimize') && KTUtil.isInResponsiveRange('desktop')) {
			...
			KTUtil.addEvent(aside, 'mouseleave', function(e) {
				...
				outsideTm = setTimeout(function() {
					if (KTUtil.hasClass(body, 'aside-minimize-hover') && KTUtil.isInResponsiveRange('desktop')) {
	...
	var initAsideMenu = function() {
		...
		var menuDesktopMode = (KTUtil.attr(menu, 'data-menu-dropdown') === '1' ? 'dropdown' : 'accordion');

		var scroll;
		if (KTUtil.attr(menu, 'data-menu-scroll') === '1') {
			scroll = {
		...
		// asideMenu.setActiveItem($('a[href="?page=custom/pages/pricing/pricing-1&demo=demo1"]').closest('.kt-menu-item')[0]);
	}

	// Sidebar toggle
	var initAsideToggler = function() {
		if (!KTUtil.get('kt_aside_toggle')) {
			return;
		}

		asideToggler = new KTToggle('kt_aside_toggle', {
			target: 'body',
			targetState: 'aside-minimize',
			togglerState: 'brand-toggle-active'
		});

		asideToggler.on('toggle', function(toggle) {
			KTUtil.addClass(body, 'aside-minimizing');
			...
			// server side code and add "kt-brand-minimize aside-minimize" classes to
			// the body tag in order to initialize the minimized left aside mode during page loading.
		});

		asideToggler.on('beforeToggle', function(toggle) {
			var body = KTUtil.get('body');
			if (KTUtil.hasClass(body, 'aside-minimize') === false && KTUtil.hasClass(body, 'aside-minimize-hover')) {
				KTUtil.removeClass(body, 'aside-minimize-hover');
			}
		});
	}
	...
	var initAsideSecondary = function() {
		...
		asideSecondaryToggler = new KTToggle('kt_aside_secondary_toggle', {
			target: 'body',
			...
			sticky: {
				...
				position: {
					top: function() {
						...
						if (KTUtil.isInResponsiveRange('desktop')) {
							if (KTUtil.hasClass(body, 'header-fixed')) {
								pos = pos + parseInt(KTUtil.css(KTUtil.get('kt_header'), 'height'));
							}

							if (KTUtil.hasClass(body, 'subheader-fixed') && KTUtil.get('kt_subheader')) {
	...
	return {
		init: function() {
			...
			$('#kt_aside_menu, #kt_header_menu').on('click', '.menu-link[href="#"]', function(e) {
				swal.fire("", "Lo sentimos esta seccion no se encuentra disponible.");
